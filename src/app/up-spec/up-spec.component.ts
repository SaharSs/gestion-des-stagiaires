...
@Component({
  selector: 'app-up-spec',
  templateUrl: './up-spec.component.html',
  styleUrls: ['./up-spec.component.css']
})
export class UpSpecComponent implements OnInit {
  keyParams:any
  dataProduct={
    name:'',
  }
  ...
  ngOnInit(): void {
    this.fs.collection('diploma').ref.doc(this.keyParams).get().then(data=>{
...
update(){
  this.fs.collection("diploma").doc(this.keyParams).update({
